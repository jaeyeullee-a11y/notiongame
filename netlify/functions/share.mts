...
async function assertUserExists(usernameLower: string): Promise<UserRecord | null> {
  const users = accountStore('stillgarden-users')
  return readJson<UserRecord>(users, usernameLower)
}
...
export default async (req: Request, _context: Context) => {
  ...
  try {
    if (req.method === 'POST') {
      ...
      const user = await assertUserExists(usernameLower)
      if (!user) {
    ...
    if (req.method === 'GET') {
      ...
      return json(200, {
        ...
        ownerUsername:
          parsed.data.ownerUsername ?? parsed.data.ownerUsernameLower,
      })
    }
